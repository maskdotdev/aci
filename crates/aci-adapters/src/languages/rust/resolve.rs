...
pub fn resolve_partition(mut partition: GraphPartition) -> GraphPartition {
    resolve_local_edges(&mut partition);
    partition
}

fn resolve_local_edges(partition: &mut GraphPartition) {
    ...
    for edge in &mut partition.edges {
        ...
        let replacement =
            GraphEdge::deterministic(edge.kind, &edge.from, target, edge.span.clone())
